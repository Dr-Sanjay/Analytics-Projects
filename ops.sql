...
SELECT t1.ds, COUNT(t1.event)/3600 AS daily_throughput, AVG(t2.throughput) AS rolling_throughput
FROM table1 t1
JOIN (
    SELECT t2.ds, COUNT(t2.event)/3600 AS throughput
    FROM table1 t2
    GROUP BY t2.ds
) AS t2 ON t1.ds >= t2.ds - INTERVAL 6 DAY AND t1.ds <= t2.ds
GROUP BY t1.ds;

-- (3). Percentage share of each language: Share of each language for different contents.
--      Your task: Calculate the percentage share of each language in the last 30 days?

SELECT language, COUNT(*) AS count, CONCAT(CAST((COUNT(*) / (SELECT COUNT(*) FROM table1)) * 100 AS DECIMAL(10,1)), '%') AS percentage
FROM table1
GROUP BY language
...
FROM (
    SELECT WEEK(occurred_at) AS Weeks, location, COUNT(user_id) AS User_engagement
    FROM events
    WHERE event_type = 'engagement'
    GROUP BY Weeks, location WITH ROLLUP
) AS e
